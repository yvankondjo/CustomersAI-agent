...
async def _upsert_whatsapp_account(
    ...
    result = db.table("social_accounts").select("*").eq("platform", "whatsapp").eq("account_id", phone_number_id or (waba_id or "")).limit(1).execute()
    if result.data:
...
async def handle_oauth_callback(
    ...
    try:
        ...
        social_account_data = {
            ...
            "account_username": profile_data["username"],
            "access_token": token_data["access_token"],
